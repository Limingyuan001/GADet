...
def area_loss(box_label,Pred_label,true_w=320,strides=[32, 16, 8],
    ...
    wh_gt = box_label[2]
    wh_pred = box_label[3]
    wh_gt_d = torch.sqrt(torch.pow(wh_gt[..., 0], 2) + torch.pow(wh_gt[..., 1], 2)).unsqueeze(-1)
    wh_pred_d = torch.sqrt(torch.pow(wh_pred[..., 0], 2) + torch.pow(wh_pred[..., 1], 2)).unsqueeze(-1)
    d_diff_gt=wh_pred_d-wh_gt_d
    loss_d_gt= torch.abs(d_diff_gt)

    loss = 28*loss_d + loss_d_gt
