...
class CBAMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = spatial_out * x
        return x
class CBAMLayer_reverse(nn.Module):
    def __init__(self, channel, reduction=16, spatial_kernel=7):
        super(CBAMLayer_reverse, self).__init__()
...
class CSPDarknet_attention(BaseModule):
    ...
    def __init__(self,
        ...
        self.cbamreverse96=CBAMLayer_reverse(96)
        self.cbamreverse192=CBAMLayer_reverse(192)
        self.cbamreverse384=CBAMLayer_reverse(384)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.layers):
            ...
            if i in self.out_indices:
                ...
                if x.size()[1]==96:
                    x=self.cbam96(x)+self.cbamreverse96(x)
                if x.size()[1]==192:
                    x=self.cbam192(x)+self.cbamreverse192(x)
                if x.size()[1]==384:
                    x=self.cbam384(x)+self.cbamreverse384(x)
                outs.append(x)
        return tuple(outs)
