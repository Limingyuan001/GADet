...
class SimOTAAssigner_cyber(BaseAssigner):
    ...
    def _assign(self,
        ...
        valid_mask, is_in_boxes_and_center = self.get_in_gt_and_in_center_info(
            priors, gt_bboxes)
        # vaild_mask （num_valid）是 proposals的掩码，初步筛选这些框是不是在中心或者gt内,
        # gts75 centers108,valid mask131 即 num_valid=131
        # is_in_boxes_and_center (num_pro,num_gt)各proposals和gt的映射关系
        ...
        # (pairwise_ious>0).sum()=102 pairwise_ious.shape ()=num_valid,num_gt
        ...
        """
        这里将pairwise_ious改成pairwise_costs对应的矩阵，然后对下面的dynamick函数
        进行修改，
        """
    ...
    def dynamic_k_matching(self, cost, pairwise_ious, num_gt, valid_mask):
        ...
        # 这里是判断是否存在prior匹配多个gt的情况，暂时先不用考虑
        ...
        #fg_mask_inboxes是num_vaild中符合dynamick条件下的掩码（131），True非常少就几个
        # 个数同valid_decoded_bbox.size（0）
        # 在这里修正valid_mask 中的真值，只有满足最终要求映射关系才能为真
