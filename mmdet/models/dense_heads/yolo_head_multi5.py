...
class YOLOV3HeadMulti5(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        super(YOLOV3HeadMulti5, self).__init__(init_cfg)
        # Check params
    ...
    def num_attrib(self):
        ...
        return 5 * self.num_classes #修改：这里将+改成了*
    ...
    def get_bboxes(self,
        ...
        for pred, stride in zip(pred_maps, self.featmap_strides):
            pred = pred.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                    self.num_attrib)
            # pred[..., :2].sigmoid_()# 这里给xy用sigmoid干嘛？？？？？
        ...
        # 新方法 面对15*5
        allbox=flatten_preds[...,:15]
        allbox[...,:2]=allbox[...,:2].sigmoid()
        allbox[..., 4]=allbox[...,4].sigmoid()
        flatten_bbox_preds=allbox[..., :4]
        flatten_objectness=allbox[..., 4]
        ...
        det_results = []
        for (bboxes,  objectness) in zip(flatten_bboxes,
                                                flatten_objectness):
            ...
            cores=torch.ones(objectness.size(0),1)
            cores=cores.cuda()
    ...
    def loss(self,
        ...
        losses_conf, losses_xy, losses_wh = multi_apply(
    ...
    def loss_single(self, pred_map, target_map, neg_map):
        ...
        # 间接法读取先读取15个类别再进行5的读取
        # 这里不在对种类进行预测而是直接使用置信度进行种类判断，不在使用pred_label
        # allbox=pred_map[...,::5]
        # pred_xy = allbox[...,:2]
        # pred_wh = allbox[..., 2:4]
        # pred_conf = allbox[..., 4]
        cuda=torch.device('cuda:0')
        pred_xy=pred_map[...,0:2]
        pred_wh=pred_map[...,2:4]
        pred_conf=torch.unsqueeze(pred_map[...,4],-1)
        # pred_map_split=torch.split(pred_map,5,dim=-1)
        for i in range(pred_map.size(-1)):
            if i>4:
                if i%5 <2 :
                    pred_xy=torch.cat((pred_xy,torch.unsqueeze(pred_map[...,i],-1)),-1)
                if 2<=i%5 and i%5<4:
                    pred_wh=torch.cat((pred_wh,torch.unsqueeze(pred_map[...,i],-1)),-1)
                if i%5==4:
                    pred_conf=torch.cat((pred_conf,torch.unsqueeze(pred_map[...,i],-1)),-1)
        # 同理target_map的读取也需要进行变化
        ...
        target_xy = target_map[..., 0:2]
        target_wh = target_map[..., 2:4]
        target_conf = torch.unsqueeze(target_map[..., 4], -1)
        for i in range(target_map.size(-1)):
            if i>4:
                if i%5 <2 :
                    target_xy=torch.cat((target_xy,torch.unsqueeze(target_map[...,i],-1)),-1)
                if 2<=i%5 and i%5<4:
                    target_wh=torch.cat((target_wh,torch.unsqueeze(target_map[...,i],-1)),-1)
                if i%5==4:
                    target_conf=torch.cat((target_conf,torch.unsqueeze(target_map[...,i],-1)),-1)
        ...
        return loss_conf, loss_xy, loss_wh
