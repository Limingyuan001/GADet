...
class CBAMLayerv2(nn.Module):
    def __init__(self, channel, reduction=16, spatial_kernel=7):
        super(CBAMLayerv2, self).__init__()
        ...
        self.sigmoid = nn.Sigmoid()
        self.relu=nn.ReLU()
        self.conv1=nn.Conv2d(channel,channel,1,padding=0,bias=False)
        self.bn1=nn.BatchNorm2d(channel)
        self.conv3=nn.Conv2d(channel,channel,3,padding=1,bias=False)
    ...
    def forward(self, x):
        ...
        spatial_out = self.sigmoid(self.conv(torch.cat([max_out, avg_out], dim=1)))
        x = spatial_out * x
        x = x + self.relu(self.bn3(self.conv3(x))) + self.relu(self.bn1(self.conv1(x)))
        return x
...
class CSPDarknet_attention(BaseModule):
    ...
    def __init__(self,
        ...
        #
        # self.freqatt96=MultiSpectralAttentionLayer(96,40,40,reduction=16,freq_sel_method='top16')
        # self.freqatt192=MultiSpectralAttentionLayer(192,20,20,reduction=16,freq_sel_method='top16')
        # self.freqatt384=MultiSpectralAttentionLayer(384,10,10,reduction=16,freq_sel_method='top16')
        # self.se96=SELayer(96)
        # self.se192=SELayer(192)
        # self.se384=SELayer(384)

        self.cbam96v2=CBAMLayerv2(96)
        self.cbam192v2=CBAMLayerv2(192)
        self.cbam384v2=CBAMLayerv2(384)
