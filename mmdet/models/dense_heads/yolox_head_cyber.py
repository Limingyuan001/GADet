...
class YOLOXHead_cyber(BaseDenseHead, BBoxTestMixin):
    ...
    def init_weights(self):
        super(YOLOXHead_cyber, self).init_weights()
    ...
    def loss(self,
        ...
        # factor_bbox=1+(loss_cls+loss_obj)/(loss_cls+loss_obj+loss_bbox)
        # factor_obj = 1 + (loss_cls + loss_bbox) / (loss_cls + loss_obj + loss_bbox)
        # factor_cls = 1 + (loss_obj + loss_bbox) / (loss_cls + loss_obj + loss_bbox)

        factor_bbox=1 + (loss_obj)/(loss_obj+loss_bbox)
        factor_obj = 1 + (loss_bbox) / (loss_obj + loss_bbox)
        # factor_cls = 1 + (loss_obj + loss_bbox) / (loss_cls + loss_obj + loss_bbox)
